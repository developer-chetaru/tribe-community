...
class MonthlySummary extends Component
{
    ...
    public function generateMonthlySummary($userId = null, $year = null, $month = null)
    {
        ...
        if ($allData->isEmpty()) {
            ...
            $dataText = $allData->map(fn ($h) => $h->created_at->format('M d').': '.
    ...
    private function calculateValidMonthsAndYears()
    {
        ...
        $lastCompletedMonthDate = now()->startOfMonth()->subMonth();
        $lastCompletedYear = (int) $lastCompletedMonthDate->year;
        $lastCompletedMonth = (int) $lastCompletedMonthDate->month;
        ...
        if ($selectedYear == $startYear && $selectedYear == $currentYear) {
            ...
            $maxMonth = $selectedYear > $lastCompletedYear ? 0 : $lastCompletedMonth;
        ...
        // If selected month is no longer valid, clamp to latest valid month.
        if (! empty($this->validMonths)) {
            $validMonthValues = array_column($this->validMonths, 'value');
            if (! in_array((int) $this->selectedMonth, $validMonthValues, true)) {
                $this->selectedMonth = (int) max($validMonthValues);
            }
        }
    }
