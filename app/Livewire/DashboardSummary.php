...
class DashboardSummary extends Component
{
    ...
    /** Calendar data: `org` = filtered org aggregate, `my` = logged-in user only (default). */
    public string $sentimentCalendarScope = 'my';
    ...
    public ?string $nextWindowTime = null;

    public ?string $checkInOpenAtIso = null;

    public ?string $checkInCloseAtIso = null;

    public bool $sentimentEligibleToday = false;
    ...
    public function loadData()
    {
        $filters = [
            ...
            'sentimentCalendarScope' => in_array($this->sentimentCalendarScope, ['org', 'my'], true)
                ? $this->sentimentCalendarScope
                : 'my',
        ];
    ...
    public function happyIndex()
    {
        try {
            ...
            $noteRules = ['moodNote' => 'nullable|string|max:500'];
            if ((int) $this->moodStatus !== 3) {
                $noteRules['moodNote'] = 'required|string|min:5|max:500';
            }
            $this->validate($noteRules, [
                'moodNote.required' => 'Please share a few words about your day.',
                'moodNote.min' => 'Description must be at least 5 characters.',
                'moodNote.max' => 'Description cannot exceed 500 characters.',
            ]);
    ...
    private function computeSentimentWindowHint(): void
    {
        $user = auth()->user();
        if (! $user) {
            $this->nextWindowTime = null;
            $this->checkInOpenAtIso = null;
            $this->checkInCloseAtIso = null;
            $this->sentimentEligibleToday = false;
        ...
        if ($user->hasRole('basecamp')) {
            $isEligibleDay = true;
        } elseif (empty($user->orgId)) {
            $isEligibleDay = in_array($dayOfWeek, ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'], true);
        } else {
            $workingDays = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'];
            $organisation = Organisation::find($user->orgId);
            if ($organisation && $organisation->working_days) {
            ...
            $isEligibleDay = in_array($dayOfWeek, $workingDays, true);
        }

        $this->nextWindowTime = null;
        $this->checkInOpenAtIso = null;
        $this->checkInCloseAtIso = null;
        $this->sentimentEligibleToday = $isEligibleDay;

        if (! $isEligibleDay) {
            return;
        }

        $open = $now->copy()->setTime(16, 0, 0);
        $close = $now->copy()->setTime(23, 59, 59);
        $this->checkInOpenAtIso = $open->format('c');
        $this->checkInCloseAtIso = $close->format('c');

        if ($now->lt($open)) {
            $this->nextWindowTime = $open->format('g:i A');
        }
    }

    private function calculateStreak(): void
    {
        $user = auth()->user();
        if (! $user) {
        ...
        $tz = \App\Helpers\TimezoneHelper::getUserTimezone($user);
        $datesWithEntry = [];
        foreach (HappyIndex::query()
            ->where('user_id', $user->id)
            ->where('status', 'active')
            ->where('created_at', '>=', Carbon::now('UTC')->subDays(400))
            ->cursor() as $entry) {
            $d = \App\Helpers\TimezoneHelper::setTimezone(Carbon::parse($entry->created_at), $tz)->toDateString();
            $datesWithEntry[$d] = true;
        }

        $cursor = \App\Helpers\TimezoneHelper::carbon(null, $tz)->subDay()->startOfDay();
        $streak = 0;
        for ($i = 0; $i < 400; $i++) {
            $d = $cursor->toDateString();
            if (! isset($datesWithEntry[$d])) {
                break;
            }
            $streak++;
            $cursor->subDay();
        ...
        if ($this->userGivenFeedback) {
            $today = \App\Helpers\TimezoneHelper::carbon(null, $tz)->toDateString();
            if (isset($datesWithEntry[$today])) {
                $streak++;
            }
        }
