	...
	public function sendForgotPasswordEmail(string $email, string $emailHtml)
{
     ...
     */
    /**
     * Delete user from OneSignal (all subscriptions, tags, and identity).
     * Call when user deletes their account so they stop receiving push/email.
     *
     * @param int|string $userId App user ID (external_id in OneSignal is "user_{$userId}")
     * @return bool
     */
    public function deleteUserByExternalId($userId): bool
    {
        if (empty($this->appId) || empty($this->restApiKey)) {
            Log::warning('⚠️ OneSignal not configured, skip delete user');
            return false;
        }

        $externalId = 'user_' . (string) $userId;

        try {
            $response = Http::withHeaders([
                'Authorization' => "Basic {$this->restApiKey}",
                'Content-Type'  => 'application/json',
            ])->delete("https://api.onesignal.com/apps/{$this->appId}/users/by/external_id/{$externalId}");

            if ($response->failed()) {
                Log::warning('⚠️ OneSignal delete user failed', [
                    'user_id' => $userId,
            ...
            Log::info('✅ OneSignal user (subscriptions) removed', [
                'user_id' => $userId,
                'external_id' => $externalId,
            ]);
            return true;
        } catch (\Throwable $e) {
            Log::error('❌ OneSignal delete user exception', [
                'user_id' => $userId,
                'error' => $e->getMessage(),
            ]);
            return false;
