...
class DashboardService
{
    ...
    public function getFreeVersionHomeDetails(array $filters = [])
    {
        ...
        $sentimentCalendarScope = $filters['sentimentCalendarScope'] ?? 'my';
        ...
        // "My Sentiment": calendar uses only the logged-in user's submissions (still respects office/dept filters on other features)
        if (! empty($orgId) && $sentimentCalendarScope === 'my') {
            $happyData = $happyData->where('user_id', $userId)->values();
        }
        ...
        for ($i = 1; $i <= $noOfDaysInMonth; $i++) {
            ...
            if (empty($orgId)) {
                ...
                if ($sentimentCalendarScope === 'org') {
                    // Org-wide sentiment for the filtered population (share of "happy" responses among all submissions that day)
            ...
            if (! empty($orgId) && $sentimentCalendarScope === 'org' && ! $isLeaveDay) {
