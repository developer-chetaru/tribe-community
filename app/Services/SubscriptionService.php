...
class SubscriptionService
{
    ...
    /**
     * Get subscriptions that have been unpaid for at least the given number of days.
     * Criteria: no paid invoice for this subscription, period end is at least $days ago.
     */
    public function getSubscriptionsUnpaidForDays(int $days): \Illuminate\Support\Collection
    {
        ...
        return SubscriptionRecord::query()
            ...
            ->get()
            ->filter(function (SubscriptionRecord $sub) {
                $hasPaid = Invoice::where('subscription_id', $sub->id)->where('status', 'paid')->exists();
                if ($hasPaid) {
                    return false;
                }
                $hasUnpaid = Invoice::where('subscription_id', $sub->id)->where('status', '!=', 'paid')->exists()
                    || ($sub->tier === 'basecamp' && $sub->user_id && Invoice::where('user_id', $sub->user_id)->where('tier', 'basecamp')->where('status', '!=', 'paid')->exists());
                return $hasUnpaid;
            });
    }

    /**
     * Delete an unpaid subscription and all related unpaid invoices and payments.
     * Used by admin manual delete and by auto-delete command. Does not delete the User.
     */
    public function deleteUnpaidSubscriptionRecord(SubscriptionRecord $subscription): bool
    {
        ...
        if ($hasPaidInvoice) {
            Log::warning("Cannot delete subscription {$subscription->id}: has paid invoices");
            return false;
        }

        $invoiceIds = Invoice::where('subscription_id', $subscription->id)->pluck('id');
        Payment::whereIn('invoice_id', $invoiceIds)->delete();
        Invoice::where('subscription_id', $subscription->id)->delete();
        ...
        $subscription->delete();
        Log::info("Unpaid subscription deleted", ['subscription_id' => $subscription->id, 'tier' => $subscription->tier]);
        return true;
    }
}
