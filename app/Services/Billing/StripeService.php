...
class StripeService
{
    ...
    /**
     * Cancel all Stripe subscriptions for a user (e.g. before account deletion).
     * Used for basecamp users who have subscription_records with user_id set.
     * Cancels immediately in Stripe so no further charges occur.
     */
    public function cancelSubscriptionsForUser(User $user): void
    {
        $subscriptions = SubscriptionRecord::where('user_id', $user->id)
            ->whereNotNull('stripe_subscription_id')
            ->get();

        foreach ($subscriptions as $subscription) {
            try {
                $this->cancelSubscription($subscription->stripe_subscription_id, false);
                Log::info('Stripe subscription cancelled for user account deletion', [
                    'user_id' => $user->id,
                    'stripe_subscription_id' => $subscription->stripe_subscription_id,
                ]);
            } catch (\Exception $e) {
                Log::warning('Failed to cancel Stripe subscription on user delete (continuing with delete)', [
                    'user_id' => $user->id,
                    'stripe_subscription_id' => $subscription->stripe_subscription_id,
                    'error' => $e->getMessage(),
                ]);
