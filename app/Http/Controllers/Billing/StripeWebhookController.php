...
class StripeWebhookController extends Controller
{
    ...
    public function handleWebhook(Request $request)
    {
        ...
        switch ($event->type) {
            ...
            case 'invoice.paid':  // Stripe API 2025+ sends this for successful invoice payment
    ...
    /**
     * Handle successful invoice payment (first payment + renewals when Stripe auto-deducts)
     */
    protected function handleInvoicePaymentSucceeded($invoice)
    {
        Log::info('Invoice payment succeeded', ['invoice_id' => $invoice->id]);

        // Subscription ID can be string or expanded object in webhook
        ...
        $subscription = SubscriptionRecord::where('stripe_subscription_id', $stripeSubscriptionId)->first();
        ...
        if (!$subscription) {
            if ($stripeCustomerId) {
                ...
                if (!$subscription && $organisation) {
                    ...
                    if ($subscription) {
                        $subscription->update(['stripe_subscription_id' => $stripeSubscriptionId]);
                        Log::info('Linked subscription to Stripe subscription via invoice.payment_succeeded', [
                            'subscription_id' => $subscription->id,
                            'stripe_subscription_id' => $stripeSubscriptionId,
                        ]);
                    }
            ...
            if (!$subscription) {
                Log::warning('Subscription not found for invoice.payment_succeeded', [
                    'stripe_subscription_id' => $stripeSubscriptionId,
        ...
        if ($isDailySubscription) {
            ...
            $updateData['current_period_end'] = \Carbon\Carbon::today()->endOfDay();
            $updateData['next_billing_date'] = \Carbon\Carbon::today()->addDay();
        } else {
            if ($stripeSubscription && isset($stripeSubscription->current_period_start) && isset($stripeSubscription->current_period_end)) {
        ...
        Log::info('Subscription updated after payment', [
            ...
            'period_end' => isset($updateData['current_period_end']) && $updateData['current_period_end'] ? $updateData['current_period_end']->format('Y-m-d') : null,
            'next_billing' => isset($updateData['next_billing_date']) && $updateData['next_billing_date'] ? $updateData['next_billing_date']->format('Y-m-d') : null,
        ]);
    }
