...
class StripeCheckoutController extends Controller
{
    ...
    public function createRenewalCheckout(Request $request)
    {
        try {
            ...
            // if user/org already has an active Stripe subscription, do not create a new checkout subscription.
            $existingSubscription = $user->hasRole('basecamp')
                ? \App\Models\SubscriptionRecord::where('user_id', $user->id)->where('tier', 'basecamp')->latest('id')->first()
                : \App\Models\SubscriptionRecord::where('organisation_id', $user->orgId)->latest('id')->first();

            if ($existingSubscription && $this->hasActiveStripeAutoRenewSubscription($existingSubscription)) {
                return response()->json([
                    'success' => true,
                    'already_active' => true,
                    'message' => 'Subscription is already active with Stripe auto-renewal. New renewal checkout is not required.',
                    'data' => [
                        ...
                        'current_period_end' => $existingSubscription->current_period_end
                            ? \Carbon\Carbon::parse($existingSubscription->current_period_end)->format('Y-m-d')
                            : null,
                    ],
                ], 200);
            }
    ...
    /**
     * Check if subscription is already active in Stripe and should continue auto-renewing.
     */
    private function hasActiveStripeAutoRenewSubscription($subscription): bool
    {
        ...
        try {
            ...
            $isActiveLike = in_array($stripeSubscription->status ?? '', $activeLikeStatuses, true);
            $periodEndInFuture = isset($stripeSubscription->current_period_end)
                ? \Carbon\Carbon::createFromTimestamp($stripeSubscription->current_period_end)->isFuture()
                : false;

            // cancel_at_period_end=true means Stripe will not auto-renew at period end.
            $autoRenewEnabled = !((bool) ($stripeSubscription->cancel_at_period_end ?? false));

            return $isActiveLike && $periodEndInFuture && $autoRenewEnabled;
        } catch (\Throwable $e) {
            Log::warning('Failed to validate existing Stripe subscription before renewal checkout', [
    ...
    public function handleSuccess(Request $request)
    {
        try {
            ...
            if ($existingPayment) {
                ...
                if ($isMobileApp) {
                    $subscription = $invoice->subscription ? $invoice->subscription->fresh() : null;
                    return response()->json([
                        'status' => true,
                        'success' => true,
            ...
            try {
                ...
                if ($session->mode === 'subscription' && $session->subscription) {
                    ...
                    try {
                        ...
                        if ($invoice->subscription) {
                            ...
                            // Create subscription if it doesn't exist (for basecamp users)
                            $paymentDate = Carbon::today();
                            $subscription = SubscriptionRecord::create([
                                'user_id' => $invoice->user_id,
                                'organisation_id' => $invoice->organisation_id,
                                'tier' => $invoice->tier ?? 'basecamp',
                                'stripe_subscription_id' => $stripeSubscriptionId,
                                'stripe_customer_id' => $stripeSubscription->customer ?? null,
                                'user_count' => $invoice->user_count ?? 1,
                                'status' => 'active',
                            ...
                            Log::info("Created new subscription for invoice", [
                                'subscription_id' => $subscription->id,
                                'invoice_id' => $invoice->id,
                                'stripe_subscription_id' => $stripeSubscriptionId,
                            ]);
                ...
                // Check if this is a mobile app request (from API or expects JSON)
                $isMobileApp = $request->wantsJson() || 
                              $request->expectsJson() || 
                              $request->is('api/*') ||
                              $request->header('Accept') === 'application/json' ||
                ...
                if ($isMobileApp) {
                    ...
                    return response()->json([
                        ...
                        'data' => [
                            ...
                            'subscription_status' => $subscription ? $subscription->status : null,
                            'subscription_period_end' => $subscription && $subscription->current_period_end 
                                ? $subscription->current_period_end->format('Y-m-d') 
                                : null,
                            'subscription_period_start' => $subscription && $subscription->current_period_start 
