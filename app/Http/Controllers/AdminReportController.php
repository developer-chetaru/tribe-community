...
class AdminReportController extends Controller
{
    ...
    public function getHappyIndexReport($perArr = [])
    {
        ...
        $indexHappyIndexOrgWith = $this->calculateHappyIndexGraph(base64_encode($orgId), '', '', $date, false, false);
        $indexHappyIndexOrgWithout = $this->calculateHappyIndexGraph(base64_encode($orgId), '', '', $date, false, true);
        foreach ($indexHappyIndexOrgWith as $idx => $happyIndexOrg) {
            $categoryId = str_replace('.', '_', $happyIndexOrg['id']);
            $withoutPct = $indexHappyIndexOrgWithout[$idx]['percentage'] ?? 0;
            $this->upsertHappyIndex($date, $orgId, $categoryId, null, null, $happyIndexOrg['percentage'], $withoutPct);
        }

        $indexHappyIndexOfficeWith = $this->calculateHappyIndexGraph(base64_encode($orgId), $officeId, '', $date, false, false);
        $indexHappyIndexOfficeWithout = $this->calculateHappyIndexGraph(base64_encode($orgId), $officeId, '', $date, false, true);
        foreach ($indexHappyIndexOfficeWith as $idx => $happyIndexOffice) {
            $categoryId = str_replace('.', '_', $happyIndexOffice['id']);
            $withoutPct = $indexHappyIndexOfficeWithout[$idx]['percentage'] ?? 0;
            $this->upsertHappyIndex($date, $orgId, $categoryId, $officeId, null, $happyIndexOffice['percentage'], $withoutPct);
        }
        ...
        foreach ($departments as $department) {
            ...
            $indexHappyIndexAllDeptWith = $this->calculateHappyIndexGraph(base64_encode($orgId), '', $allDepartmentId, $date, false, false);
            $indexHappyIndexAllDeptWithout = $this->calculateHappyIndexGraph(base64_encode($orgId), '', $allDepartmentId, $date, false, true);
            foreach ($indexHappyIndexAllDeptWith as $idx => $happyIndexAllDept) {
                $categoryId = str_replace('.', '_', $happyIndexAllDept['id']);
                $withoutPct = $indexHappyIndexAllDeptWithout[$idx]['percentage'] ?? 0;
                $this->upsertHappyIndex($date, $orgId, $categoryId, null, $allDepartmentId, $happyIndexAllDept['percentage'], $withoutPct);
            }

            // Specific Department + Office
            $indexHappyIndexDeptWith = $this->calculateHappyIndexGraph(base64_encode($orgId), $officeId, $departmentId, $date, false, false);
            $indexHappyIndexDeptWithout = $this->calculateHappyIndexGraph(base64_encode($orgId), $officeId, $departmentId, $date, false, true);
            foreach ($indexHappyIndexDeptWith as $idx => $happyIndexDept) {
                $categoryId = str_replace('.', '_', $happyIndexDept['id']);
                $withoutPct = $indexHappyIndexDeptWithout[$idx]['percentage'] ?? 0;
                $this->upsertHappyIndex($date, $orgId, $categoryId, $officeId, $departmentId, $happyIndexDept['percentage'], $withoutPct);
            }
        }
    }
     ...
     * @return void
     */
    private function upsertHappyIndex($date, $orgId, $categoryId, $officeId = null, $departmentId = null, $withWeekend = null, $withoutWeekend = null)
    {
        ...
        $data = [
            ...
            'without_weekend' => $withoutWeekend,
            'updated_at' => now(),
        ];
     ...
     */
    public function calculateHappyIndexGraph($orgIdEncoded, $officeId, $departmentId, $date, $categoryId = false, bool $excludeWeekend = false)
    {
        ...
        foreach ($happyIndexMoodValues as $moodVal) {
            ...
            if ($excludeWeekend) {
                ...
                if ($driver === 'mysql') {
                    $moodCountQuery->whereRaw('WEEKDAY(happy_indexes.created_at) < 5');
                } elseif ($driver === 'sqlite') {
                    $moodCountQuery->whereRaw("CAST(strftime('%w', happy_indexes.created_at) AS INTEGER) BETWEEN 1 AND 5");
                } elseif ($driver === 'pgsql') {
                    $moodCountQuery->whereRaw('EXTRACT(DOW FROM happy_indexes.created_at)::int BETWEEN 1 AND 5');
                }
            }
