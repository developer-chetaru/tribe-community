...
class MonthlySummaryController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!$user && $token) {
            \Illuminate\Support\Facades\Log::warning("MonthlySummary: User not found from token", [
            ...
            try {
                ...
                if (count($parts) === 3) {
                    $payload = json_decode(base64_decode(str_replace(['-', '_'], ['+', '/'], $parts[1])), true);
                    $userId = $payload['sub'] ?? null;
                    \Illuminate\Support\Facades\Log::info("MonthlySummary: Manual decode result", [
                    ...
                    if ($userId) {
                        ...
                        $user = \App\Models\User::find($userId);
                        if ($user) {
                            \Illuminate\Support\Facades\Log::info("MonthlySummary: User found via manual decode", [
                                'user_id' => $user->id,
                            ]);
                        } else {
                            \Illuminate\Support\Facades\Log::warning("MonthlySummary: User not found in database", [
                                'user_id' => $userId,
                                'user_id_type' => gettype($userId),
                            ]);
                        }
                ...
                \Illuminate\Support\Facades\Log::error("MonthlySummary: Manual decode failed", [
                    'error' => $e->getMessage(),
                ]);
            }
        }
