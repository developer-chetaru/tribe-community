...
class SummaryController extends Controller
{
    ...
    public function getSummary($filterType, Request $request)
    {
        ...
        foreach ($period as $date) {
            ...
            // Check if leave
            $onLeave = $leaves->first(fn ($l) => $date->between(Carbon::parse($l->start_date), Carbon::parse($l->end_date))
            );
            ...
            if ($entry) {
                ...
                $image = match ((int) $entry->mood_value) {
                    ...
                    2 => $imageUrl('average-app.png'),
                    1 => $imageUrl('sad-app.png'),
                ...
                if ($shouldShowMissed) {
                    $summary[] = [
                        ...
                        'image' => $imageUrl('sentiment-missed-yellow.svg'),
                        'status' => 'Missed',
                    ];
