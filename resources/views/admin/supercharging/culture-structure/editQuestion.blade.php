<x-app-layout>
    <x-slot name="header">
        <h2 class="text-[24px] md:text-[30px] font-semibold text-[#EB1C24]">
            Edit Culture Structure Question
        </h2>
    </x-slot>
    ...
    <div class="bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('admin.supercharging.culture-structure.questions.update', $question->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Options (4 required - one for each culture type) <span class="text-red-500">*</span></label>
                <p class="text-sm text-gray-500 mb-4">Each option must be assigned to one of the 4 culture types.</p>
                
                <div id="options-container" class="space-y-4">
                    @foreach($cultureTypes as $index => $cultureType)
                        @php
                            $existingOption = $question->options->where('culture_type_id', $cultureType->id)->first();
                        @endphp
                        <div class="border border-gray-300 rounded p-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Option for {{ $cultureType->title }} <span class="text-red-500">*</span>
                            </label>
                            @if($existingOption)
                                <input type="hidden" name="options[{{ $index }}][id]" value="{{ $existingOption->id }}">
                            @endif
                            <input type="hidden" name="options[{{ $index }}][culture_type_id]" value="{{ $cultureType->id }}">
                            <input type="text" name="options[{{ $index }}][text]" required
                                   value="{{ old("options.$index.text", $existingOption->option_text ?? '') }}"
                                   placeholder="Enter option text for {{ $cultureType->title }}..."
                                   class="w-full border border-gray-300 rounded px-4 py-2 focus:ring-2 focus:ring-red-500 focus:border-red-500">
                        </div>
                    @endforeach
                </div>
