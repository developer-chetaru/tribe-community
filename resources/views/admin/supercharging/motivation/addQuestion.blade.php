<x-app-layout>
    ...
    <div class="bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('admin.supercharging.motivation.questions.store') }}" method="POST">
            @csrf
                    ...
                    @for($i = 0; $i < 2; $i++)
                        <div class="border border-gray-300 rounded p-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Option {{ $i === 0 ? 'A' : 'B' }} <span class="text-red-500">*</span>
                            </label>
                            ...
                            <select name="options[{{ $i }}][motivation_value_id]" required
                                    class="w-full border border-gray-300 rounded px-4 py-2 focus:ring-2 focus:ring-red-500 focus:border-red-500">
                                <option value="">-- Select Motivation Value --</option>
                                @foreach($motivationValues as $value)
                                    <option value="{{ $value->id }}" 
                                            {{ old("options.$i.motivation_value_id") == $value->id ? 'selected' : '' }}>
                                        {{ $value->title }} ({{ $value->value_key }})
                                    </option>
                                @endforeach
                            </select>
                            
                            <input type="hidden" name="options[{{ $i }}][label]" value="Option {{ $i === 0 ? 'A' : 'B' }}">
                        </div>
                    @endfor
                </div>
            </div>
