        ...
        <table class="w-full border-collapse border border-gray-200 text-sm">
            <thead>
                <tr class="bg-gray-100">
                    ...
                    <th class="border border-gray-300 px-4 py-2 text-left font-semibold">Role Key</th>
                    <th class="border border-gray-300 px-4 py-2 text-left font-semibold">Title</th>
                    <th class="border border-gray-300 px-4 py-2 text-left font-semibold">Value Focus</th>
            ...
            <tbody>
                @forelse($descriptions as $description)
                    <tr class="border border-gray-300 hover:bg-red-50">
                        <td class="px-4 py-2">{{ $description->order }}</td>
                        <td class="px-4 py-2 font-mono text-xs">{{ $description->role_key }}</td>
                        <td class="px-4 py-2 font-semibold">{{ $description->title }}</td>
                        <td class="px-4 py-2">{{ $description->value_focus }}</td>
                        <td class="px-4 py-2">
                            <span class="px-2 py-1 rounded text-xs {{ $description->status == 'Active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                {{ $description->status }}
                            </span>
                        </td>
                        <td class="px-4 py-2 flex gap-2 justify-end">
                            <a href="{{ route('admin.cot.team-role-descriptions.edit', $description->id) }}"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-4 text-gray-500">No role descriptions found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
