<x-app-layout>
    ...
    <div class="bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('admin.cot.questions.update', $question->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <div id="options-container" class="space-y-4">
                    @foreach($question->roleMapOptions as $index => $option)
                        <div class="border border-gray-300 rounded p-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Option {{ $index + 1 }} <span class="text-red-500">*</span></label>
                            <input type="hidden" name="options[{{ $index }}][id]" value="{{ $option->id }}">
                            <input type="text" name="options[{{ $index }}][text]" required
                                   value="{{ old("options.$index.text", $option->maper) }}"
                            ...
                            <select name="options[{{ $index }}][role_key]" required
                                    class="w-full border border-gray-300 rounded px-4 py-2 focus:ring-2 focus:ring-red-500 focus:border-red-500">
                                <option value="">-- Select Role --</option>
                                @foreach($roleDescriptions as $role)
                                    <option value="{{ $role->role_key }}" 
                                            {{ old("options.$index.role_key", $option->roleDescription->role_key ?? '') == $role->role_key ? 'selected' : '' }}>
