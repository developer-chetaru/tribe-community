    ...
    <div class="bg-white p-6 rounded shadow">
        ...
        @if($completionStatus)
            <div class="mb-4 px-4 py-2 bg-blue-100 border border-blue-400 text-blue-700 rounded">
                ...
                <a href="{{ route('tribeometer.results') }}" class="text-blue-600 underline mt-2 inline-block">
                    View Results
                </a>
            </div>
        @endif
        ...
        <div class="mb-4">
            <p class="text-gray-600 mb-4">
                Help us understand how well our organizational values resonate with you. 
                Select the option that best represents your level of agreement with each statement.
            </p>
                    ...
                    @foreach($options as $option)
                        <div class="text-center">
                            <span class="font-semibold 
                                {{ $option->value_score == 0 ? 'text-red-600' : 
                                   ($option->value_score == 1 ? 'text-orange-600' : 
                                   ($option->value_score == 2 ? 'text-blue-600' : 'text-green-600')) }}">
                                {{ $option->value_score }}
                            </span>
                            <p class="text-gray-600 text-xs">{{ Str::limit($option->option_name, 20) }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <form action="{{ route('tribeometer.submit') }}" method="POST" id="tribeometerForm">
            @csrf
            
            @php
                $questionsByValue = $questions->groupBy(function($question) {
                    return $question->value->title ?? 'Uncategorized';
                });
            @endphp

            @foreach($questionsByValue as $valueTitle => $valueQuestions)
                <div class="mb-8">
                    <h3 class="text-xl font-bold text-[#EB1C24] mb-4 pb-2 border-b-2 border-red-200">
                        {{ $valueTitle }}
                    </h3>
                    
                    @foreach($valueQuestions as $questionIndex => $question)
                        @php
                                    ...
                                    <label class="flex items-center p-4 border-2 rounded-lg hover:bg-gray-50 cursor-pointer transition-all
                                        ...
                                        <div class="ml-4 flex-1">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <span class="font-medium text-gray-800 text-sm">{{ $option->option_name }}</span>
                                            </div>
                                        </div>
                                    </label>
                                @endforeach
            ...
            <div class="flex gap-4 mt-6 sticky bottom-0 bg-white p-4 border-t border-gray-200 rounded-t-lg shadow-lg">
                ...
                @if($completionStatus)
                    <a href="{{ route('tribeometer.results') }}" 
